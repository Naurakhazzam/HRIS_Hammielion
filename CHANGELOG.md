...
### 122. Fitur: Halaman Aturan Potongan Gaji (Transparansi ke Semua Karyawan)

**Konteks:** Owner minta aturan potongan (hasil #121) juga bisa dilihat langsung di aplikasi, bukan cuma di penjelasan chat — supaya tiap karyawan paham konsekuensi uangnya sendiri, dan Owner/HR bisa pantau status semua orang sekaligus.

**Fitur — halaman baru `/potongan` ("Aturan Potongan Gaji"), muncul di sidebar semua role:**

- **Tampilan Karyawan (lihat data diri sendiri saja):**
  - Kartu **sumber gaji harian**: rincian Gaji Pokok + semua tunjangan ÷ 26 = gaji harian, supaya transparan dari mana angka potongan berasal (pakai RPC `get_my_salary_summary()` — karyawan tetap tidak bisa baca tabel `salary_components` langsung, cuma angka turunannya sendiri).
  - Tabel **Rupiah nyata** per tingkat kejadian untuk kelompok Izin/Sakit-tanpa-surat (5 tingkat) dan Alpha (6 tingkat), dihitung dari gaji harian masing-masing — bukan cuma angka pengali abstrak.
  - Rincian Sakit Dengan Surat Dokter (gratis/0.5×/1× per hari) dalam Rupiah.
  - Tarif **Terlambat** (Rp1.000/menit, universal) dan **Lembur** (Rp7.500/jam) beserta status berlaku/tidaknya lembur untuk penempatan orang itu (mengikuti aturan lembur yang sudah ada: flag per-karyawan + pengecualian Team Gudang).
  - **Status periode berjalan** (26-25 saat ini): jumlah kejadian Izin/Alpha sejauh ini beserta breakdown tanggal & totalnya, supaya karyawan sadar sebelum periode ditutup.
- **Tampilan Owner/HR/Finance (lihat semua karyawan):** tabel semua karyawan aktif dengan gaji harian, jumlah kejadian Izin & Alpha periode berjalan, total potongan sejauh ini, dan tombol Detail yang membuka kartu rincian lengkap yang sama persis dengan yang dilihat karyawan itu sendiri (supaya angkanya konsisten).

**Implementasi teknis:**
- RPC baru `get_my_salary_summary()` (SECURITY DEFINER, sempit — cuma kembalikan angka turunan milik pemanggil sendiri) menggantikan `get_my_daily_rate()` yang lebih simpel.
- Helper baru `getCurrentPeriodRangeStr()` di `lib/rosterPeriod.ts` untuk "periode 26-25 yang sedang berjalan sekarang" (beda dari `getUpcomingRosterPeriod()` yang dipakai Ajukan Libur untuk periode berikutnya) — `cuti/ajukan/page.tsx` dirapikan supaya pakai helper ini juga (sebelumnya ada duplikat fungsi lokal).
- Pakai ulang `calcEscalatingDeduction()` dari `lib/escalatingDeduction.ts` (sama seperti Slip Gaji Bulanan) supaya hasilnya selalu konsisten dengan potongan payroll asli.

| File | Perubahan |
|---|---|
| Migrasi DB (Supabase) | RPC `get_my_salary_summary()` baru (drop `get_my_daily_rate()`) |
| `app/(dashboard)/potongan/page.tsx` (baru) | Halaman Aturan Potongan Gaji, 2 tampilan berbeda per role |
| `lib/rosterPeriod.ts` | Tambah `getCurrentPeriodRangeStr()` |
| `app/(dashboard)/cuti/ajukan/page.tsx` | Pakai `getCurrentPeriodRangeStr()` dari helper bersama (hapus duplikat) |
| `components/sidebar.tsx` | Tambah menu "Aturan Potongan Gaji" di kedua daftar nav (admin & karyawan) |

**Fix susulan:** halaman ini belum mengecek cookie "Preview Tampilan Karyawan" (`lib/previewMode.ts`), jadi saat Owner memakai mode Preview, halaman tetap mendeteksi role asli dan menampilkan tabel SEMUA karyawan — bukan cuma data diri sendiri seperti seharusnya di mode Preview. Ditambahkan pengecekan `isPreviewModeClient()` supaya konsisten dengan halaman lain (mis. Dashboard).

---

### 123. Preview Tampilan Karyawan Sekarang Pakai Contoh Nyata (Rahmat Saleh)

**Konteks:** Setelah fix #122, Owner minta lebih jauh: mode Preview sebaiknya tidak menampilkan data akun admin sendiri (yang biasanya kosong/tidak representatif karena Owner bukan staff operasional), tapi data **karyawan sungguhan** supaya preview benar-benar menggambarkan apa yang dilihat karyawan asli. Owner memilih **Rahmat Saleh (EMP-013)** sebagai contoh karena datanya paling lengkap, dan minta cakupannya ke SEMUA halaman Portal, bukan cuma Dashboard/Aturan Potongan Gaji.

**Perubahan:**
- `lib/previewMode.ts`: tambah konstanta `PREVIEW_EMPLOYEE_ID` (id Rahmat Saleh). Semua halaman yang menghormati mode Preview sekarang mengganti employee_id yang dipakai untuk QUERY BACA ke Rahmat Saleh, bukan employee_id akun admin yang login.
- **Risiko yang diantisipasi:** RPC/insert untuk AKSI (submit pengajuan, edit profil, respon tukar libur, dst) tetap menyasar identitas akun admin yang benar-benar login (tidak bisa "dipalsukan" jadi Rahmat Saleh dari client) — kalau dibiarkan aktif, submit saat preview bisa menghasilkan data nyata yang keliru (misal pengajuan libur atas nama admin padahal terlihat seperti punya Rahmat Saleh). Karena itu **semua tombol submit/aksi di halaman Portal dinonaktifkan (read-only) selama mode Preview aktif**, dengan banner penjelasan di tiap halaman.
- Halaman yang disesuaikan (baca data Rahmat Saleh saat preview):
  - **Dashboard** — kartu jadwal hari ini, sisa cuti, kasbon, tidak absen.
  - **Aturan Potongan Gaji** (`/potongan`) — pakai query tabel langsung (bukan RPC `get_my_salary_summary()`, yang selalu balikin data pemanggil sendiri) karena admin sudah punya akses baca penuh lewat RLS.
  - **Slip Gaji Saya, Rekap Absensi Saya, Jadwal Saya** — murni baca, tidak ada tombol aksi, jadi tinggal ganti employee_id saja.
  - **Ajukan Jadwal Libur** — baca data Rahmat Saleh (termasuk hitung ulang kuota weekend per cabang tanpa RPC `get_my_branch_employee_count()`, karena RPC itu juga selalu berbasis pemanggil sendiri); tombol Pilih/Batalkan/Ajukan ke HR dinonaktifkan.
  - **Ganti Hari Libur** — baca `roster_change_requests` langsung (bukan RPC `get_my_day_off_change_requests()`/`get_incoming_day_off_swap_requests()`, karena keduanya SECURITY DEFINER berbasis pemanggil sendiri) memakai akses baca owner/hr yang sudah ada; tombol Ajukan/Terima/Tolak/Batalkan dinonaktifkan.
  - **Profil Saya** — tampilkan profil Rahmat Saleh, tombol Edit disembunyikan (RPC `update_own_employee_profile` selalu mengubah profil akun admin sendiri, bukan Rahmat Saleh, jadi berbahaya kalau dibiarkan aktif).
- Banner global di `DashboardShell.tsx` diperbarui: sekarang menyebutkan eksplisit "contoh nyata milik Rahmat Saleh" dan bahwa submit/aksi dinonaktifkan.

**Catatan batasan:** role `finance` tidak punya akses baca langsung ke `employee_roster`/`leave_requests` (memang dibatasi sejak awal, beda dari owner/hr) — kalau finance memakai Preview, kartu jadwal/cuti di Dashboard & Jadwal Saya bisa tampil kosong untuk bagian itu; ini konsisten dengan batasan RLS yang sudah ada sebelumnya, bukan bug baru.

| File | Perubahan |
|---|---|
| `lib/previewMode.ts` | Tambah `PREVIEW_EMPLOYEE_ID` (Rahmat Saleh) |
| `components/DashboardShell.tsx` | Banner Preview diperbarui |
| `app/(dashboard)/dashboard/page.tsx` | Data kartu pakai Rahmat Saleh saat preview |
| `app/(dashboard)/potongan/page.tsx` | Tambah `fetchPreviewStatus()` (query langsung, bukan RPC) |
| `app/(dashboard)/portal/absensi/page.tsx`, `slip-gaji/page.tsx`, `jadwal/page.tsx` | Ganti employee_id ke Rahmat Saleh saat preview |
| `app/(dashboard)/portal/ajukan-libur/page.tsx` | Baca data Rahmat Saleh + nonaktifkan Pilih/Batalkan/Ajukan saat preview |
| `app/(dashboard)/portal/ganti-libur/page.tsx` | Baca `roster_change_requests` langsung saat preview + nonaktifkan semua aksi |
| `app/(dashboard)/portal/profil/page.tsx` | Tampilkan profil Rahmat Saleh + sembunyikan tombol Edit saat preview |

---

*Terakhir diupdate: Sesi 7 (2026-09-18 s/d 20) — fitur Lupa Password + fix 4 bug + redesain menu karyawan + undangan interview seragam + tutup akses data rekening + fix race condition cuti + panel filter pelamar + sort jarak & kesan tes + info lokasi training + tabel rekap konfirmasi interview + jalur kedua ke rekap + auto-advance status interview + form hasil interview + fix bug limit 1000 baris + peringatan pending + Saldo Real vs Proyeksi Cash Flow + rombak ledger Supplier + fix sticky header modal supplier + fitur Catatan Meeting + fix baris Kelola Pembelian + modal diperlebar & Aksi di tabel ledger + rombak sidebar 4 kelompok + hapus menu Laporan + tutup 2 jalur bocor Kas Keluar + approval wajib kasbon driver/kenek + fix RLS terbuka + fix /signup tidak bisa diakses + fitur Preview Tampilan Karyawan + penyempurnaan portal karyawan (keamanan RLS + fitur baru) + fitur tukar hari libur saat absen masuk + fitur Absen QR menggantikan sementara Absen HP GPS + fitur Daftar Cepat kode karyawan saja + fitur Perbarui Akun Saya (email standar + password mandiri) + Portal Saya untuk semua role + fix Absen QR gagal tersimpan + fix tombol Ambil Foto tidak berfungsi + fix layar kamera hitam + jalur cadangan kamera bawaan HP + fix Kode Karyawan case-sensitive + pesan error kamera lebih detail + fix macet di Menyiapkan Kamera + cetak QR 2 per halaman lebih besar + cetak QR 1/halaman & pilih cabang + verifikasi Daftar Cepat via nama+tanggal lahir + jalur cadangan Kode Karyawan saja + edit data pribadi sendiri di Profil Saya + lihat akun baru daftar di Manajemen User + upload foto profil sendiri + fix RLS foto karyawan terbuka + tampilkan email login di Profil Saya + karyawan bisa merangkap jabatan Driver + rombak verifikasi Sudah Jadi Karyawan tanpa Kode Karyawan + fix foto absen tidak muncul di mode Semua Karyawan + konfirmasi eksplisit absen masuk/pulang + pengaman jarak waktu absen + kartu Tidak Absen di dashboard + fitur Pengajuan Jadwal Libur Awal + fix kasbon driver/kenek tidak muncul di Verifikasi Keuangan + fitur Pengajuan Ganti Hari Libur dengan tukar mutual + fix bug edit keterlambatan tidak tersimpan + toleransi 5 menit absen QR + auto-promosi training ke staff tetap + gaji standar staff/Team Toko & tunjangan khusus + fix pengajuan jadwal libur awal tidak muncul di persetujuan + batas 1 hari weekend per periode untuk pengajuan libur + konfirmasi sebelum batalkan pengajuan libur + tombol Batalkan merah solid + fitur Undang Karyawan Baru via link sekali pakai + wajib isi penuh 4/4 sebelum diajukan ke HR + wajib H-2 cuti/izin pilihan otomatis Alpha kalau dadakan + rombak potongan eskalasi per kejadian & tarif telat universal + halaman Aturan Potongan Gaji transparan ke semua karyawan + Preview Tampilan Karyawan pakai contoh nyata Rahmat Saleh (read-only)*
