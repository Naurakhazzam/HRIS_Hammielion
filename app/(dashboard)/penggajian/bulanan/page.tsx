...
export default function PenggajianBulananPage() {
  ...
  // payTargets kosong = modal tertutup. Diisi 1 slip (aksi satuan) atau banyak (aksi massal).
  const [payTargets, setPayTargets] = useState<Payroll[]>([])
  ...
  // Jam yang benar-benar dihitung ke gaji per hari — defaultnya sama dengan jam tercatat mesin,
  // tapi bisa disesuaikan manual (mis. surat lembur cuma approve 2 dari 3 jam yang tercatat).
  const [otAdjusted, setOtAdjusted]     = useState<Record<string, number>>({})
  ...
  async function openCreateModal() {
    setCreateStep(1); setSelectedEmpId(''); setUnclassifiedDays([]); setOtCandidates([]); setOtChecked({}); setOtAdjusted({}); setCreateKasbon(0); setSlipPreview(null)
  ...
  // validatedOtDates: date -> jam yang dihitung ke gaji (bisa beda dari jam tercatat mesin,
  // kalau HR sesuaikan manual di checklist). Cuma tanggal yang ADA di map ini yang dihitung.
  async function buildSlipPreview(empId: string, kasbonDed: number, validatedOtDates: Map<string, number>, opts?: { silent?: boolean }) {
  ...
  async function handleFinalizeSlip() {
    ...
    if (error) {
      ...
      // Simpan hasil checklist validasi lembur ke attendances (termasuk jam yang sudah
      // disesuaikan manual, kalau beda dari yang tercatat mesin), supaya rincian lembur di
      // slip (dan slip yang dicetak) nanti sinkron dengan yang benar-benar dibayar.
      const validDays = otCandidates.filter(d => otChecked[d.date])
      const invalidDates = otCandidates.filter(d => !otChecked[d.date]).map(d => d.date)
      await Promise.all(validDays.map(d =>
        supabase.from('attendances')
          .update({ overtime_validated: true, overtime_hours_adjusted: otAdjusted[d.date] ?? d.hours })
          .eq('employee_id', p.employeeId).eq('date', d.date)
      ))
      if (invalidDates.length > 0) {
        await supabase.from('attendances').update({ overtime_validated: false, overtime_hours_adjusted: null }).eq('employee_id', p.employeeId).in('date', invalidDates)
      }
  ...
  // Dipakai baik untuk satu slip (aksi satuan) maupun banyak slip sekaligus (aksi massal).
  function openPayModal(targets: Payroll[]) {
    const total = targets.reduce((s, p) => s + Number(p.net_total), 0)
    setPayTargets(targets)
    setPayDate(todayLocalStr())
    setPaySources([{ account_id: '', amount: String(total) }])
  }
  ...
  // Bisa dipakai untuk 1 slip (payTargets.length === 1) atau banyak slip sekaligus (aksi massal).
  const payTotal = payTargets.reduce((s, p) => s + Number(p.net_total), 0)
  const paySourcesTotal = paySources.reduce((s, r) => s + (parseFloat(r.amount) || 0), 0)
  const paySourcesValid = payTargets.length > 0
    && payDate !== ''
    && paySources.length > 0
    && paySources.every(r => r.account_id && (parseFloat(r.amount) || 0) > 0)
    && paySourcesTotal === payTotal
  ...
  async function confirmPaySlip() {
    ...
    // (sudah sekaligus jadi otorisasi/verifikasi). Tanggal transaksi = tanggal pembayaran yang
    // diisi user (default hari ini), BUKAN periode gaji — konsisten dengan aturan pencatatan
    // Kas Keluar lain: dicatat di bulan uang benar-benar keluar, bukan bulan periode gaji.
    // Kalau sumbernya lebih dari satu, tiap karyawan dialokasikan proporsional ke tiap sumber
    // sesuai porsi yang diisi user untuk total gabungan — sumber terakhir menampung sisa
    // pembulatan supaya total per karyawan selalu persis pas.
    const cashOutRows: any[] = []
    for (const p of succeeded) {
      const periodLabel = getPeriodLabel(p.period_month, p.period_year)
      let remaining = Math.round(Number(p.net_total))
      paySources.forEach((r, idx) => {
        const isLast = idx === paySources.length - 1
        const srcAmount = parseFloat(r.amount) || 0
        const portion = isLast ? remaining : Math.round(srcAmount * Number(p.net_total) / payTotal)
        remaining -= portion
        if (portion > 0) {
          cashOutRows.push({
    ...
    let coError: string | null = null
    if (cashOutRows.length > 0) {
      const { error } = await supabase.from('fin_cash_out').insert(cashOutRows)
    ...
    if (failedIds.length > 0) {
      ...
      showMessage('success', `${succeeded.length} slip ditandai lunas dan pengeluaran berhasil dicatat di Kas Keluar.`)
    }
  ...
  async function handleBulkStatusChange(action: 'ajukan' | 'approve' | 'paid') {
    ...
    // Tandai Lunas massal — buka modal metode & sumber pembayaran yang sama seperti aksi
    // satuan, supaya tetap langsung tercatat di Kas Keluar (bukan confirm() biasa).
    if (action === 'paid') {
      if (skipped > 0) showMessage('error', `${skipped} slip dilewati karena statusnya belum "Disetujui".`)
      openPayModal(eligible)
      return
    }
  ...
  async function fetchOtDetails(p: Payroll) {
    ...
    // Pakai overtime_hours_adjusted kalau HR sempat menyesuaikan manual saat validasi.
    const { data: atts } = await supabase.from('attendances').select('date, overtime_hours, overtime_hours_adjusted').eq('employee_id', p.employee_id).eq('overtime_validated', true).gte('date', effectiveStart).lte('date', lastDay).order('date')

    const details = (atts || []).map(a => {
      let hours: number
      if (a.overtime_hours_adjusted != null) {
        hours = Number(a.overtime_hours_adjusted)
      } else {
        const mins = Number(a.overtime_hours) * 60
        hours = mins < 60 ? 0 : Math.floor(mins / 60)
      }
      return { date: a.date, hours, raw: Number(a.overtime_hours), earning: hours * otRate }
    }).filter(d => d.hours > 0)
          ...
          <table className="w-full text-left border-separate border-spacing-0 text-sm">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase whitespace-nowrap sticky top-0 left-0 z-30 bg-white border-b border-slate-200 shadow-[2px_0_4px_-1px_rgba(0,0,0,0.08)]">
                  <input type="checkbox"
                ...
                <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase whitespace-nowrap sticky top-0 z-20 bg-white border-b border-slate-200">Periode</th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-right whitespace-nowrap sticky top-0 z-20 bg-white border-b border-slate-200">Gaji Pokok</th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-right whitespace-nowrap sticky top-0 z-20 bg-white border-b border-slate-200">Total Tunjangan</th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-right whitespace-nowrap sticky top-0 z-20 bg-white border-b border-slate-200">Lembur</th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-right whitespace-nowrap sticky top-0 z-20 bg-white border-b border-slate-200">Bonus KPI</th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-right whitespace-nowrap sticky top-0 z-20 bg-white border-b border-slate-200">Pot. Kasbon</th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-right whitespace-nowrap sticky top-0 z-20 bg-white border-b border-slate-200">Pot. Telat</th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-right whitespace-nowrap sticky top-0 z-20 bg-white border-b border-slate-200">Total Bersih</th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-center whitespace-nowrap sticky top-0 z-20 bg-white border-b border-slate-200">Status</th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-center whitespace-nowrap print-hide sticky top-0 z-20 bg-white border-b border-slate-200">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-xl w-full max-w-lg my-8">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500">
                {payTargets.length === 1
                  ? `${payTargets[0].employee?.full_name} · ${getPeriodLabel(payTargets[0].period_month, payTargets[0].period_year)}`
                  : `${payTargets.length} karyawan terpilih`}
              </p>
            </div>
            <button onClick={() => setPayTargets([])} className="text-slate-400 hover:text-slate-600 text-xl leading-none">×</button>
          </div>
          ...
          <div className="px-6 py-4 space-y-4">
              ...
              <div className="max-h-32 overflow-y-auto border border-slate-100 rounded-lg divide-y divide-slate-50">
                {payTargets.map(p => (
                  <div key={p.id} className="px-3 py-1.5 flex items-center justify-between text-xs">
                    ...
                    <span className="text-slate-500">{formatRupiah(Number(p.net_total))}</span>
                  </div>
                ))}
              </div>
            )}
            <div className="bg-blue-50 border border-blue-100 rounded-lg px-4 py-3 flex items-center justify-between">
              ...
              <span className="text-lg font-bold text-blue-800">{formatRupiah(payTotal)}</span>
            </div>
            ...
            <div className={`rounded-lg px-4 py-2.5 flex items-center justify-between text-sm ${paySourcesTotal === payTotal ? 'bg-green-50 border border-green-200 text-green-700' : 'bg-amber-50 border border-amber-200 text-amber-700'}`}>
              <span>Total sumber dimasukkan</span>
              <span className="font-bold">{formatRupiah(paySourcesTotal)}{paySourcesTotal !== payTotal && ` (kurang ${formatRupiah(payTotal - paySourcesTotal)})`}</span>
            </div>
          </div>

          <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-3">
            <button onClick={() => setPayTargets([])} className="px-4 py-2 text-sm text-slate-600 border border-slate-300 rounded-lg hover:bg-slate-50">Batal</button>
              ...
              <>
                  ...
                  <div className="space-y-3">
                    ...
                    {otApplicable && (
                      <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg space-y-2">
                        ...
                        <p className="text-xs text-slate-500">Cocokkan dengan surat lembur fisik yang sudah di-ACC. Yang tidak dicentang tidak ikut dihitung ke gaji. Jam yang dihitung bisa disesuaikan manual kalau beda dari yang tercatat mesin.</p>
                        {loadingOtCandidates ? (
                          ...
                          <div className="space-y-1.5 mt-1">
                            {otCandidates.map(d => (
                              <div key={d.date} className={`flex items-center justify-between p-2 rounded-lg border transition ${otChecked[d.date] ? 'bg-green-50 border-green-300' : 'bg-white border-slate-200'}`}>
                                <label className="flex items-center gap-2 cursor-pointer flex-1">
                                  ...
                                  <span className="text-[10px] text-slate-400">(tercatat {d.hours} jam)</span>
                                </label>
                                <div className="flex items-center gap-1">
                                  <input type="number" min="0" step="0.5" disabled={!otChecked[d.date]}
                                    value={otAdjusted[d.date] ?? d.hours}
                                    onChange={e => setOtAdjusted(prev => ({ ...prev, [d.date]: Math.max(0, parseFloat(e.target.value) || 0) }))}
                                    className="w-16 px-2 py-1 border border-slate-300 rounded text-sm text-right disabled:bg-slate-100 disabled:text-slate-400" />
                                  <span className={`text-sm font-semibold ${otChecked[d.date] ? 'text-green-600' : 'text-slate-400'}`}>jam</span>
                                </div>
                              </div>
                            ))}
                            <div className="flex justify-between items-center pt-1 border-t border-slate-200">
                              ...
                              <span className="text-sm font-bold text-green-600">
                                {otCandidates.filter(d => otChecked[d.date]).reduce((s, d) => s + (otAdjusted[d.date] ?? d.hours), 0)} jam
                              </span>
                            </div>
                          </div>
                        )}
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <button disabled={!selectedEmpId || buildingPreview || checkingUnclassified || unclassifiedDays.length > 0 || loadingOtCandidates}
                    onClick={async () => {
                      const validatedDates = new Map(otCandidates.filter(d => otChecked[d.date]).map(d => [d.date, otAdjusted[d.date] ?? d.hours]))
