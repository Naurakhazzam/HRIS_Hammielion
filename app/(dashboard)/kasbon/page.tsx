...
type HelperKasbonDeductionRecord = {
  id: string
  kasbon_id: string
  helper_id: string
  week_start_date: string
  ...
  remaining_after: number
  created_at: string
  helper_kasbon?: { notes: string | null; total_amount: number }
  employees?: { full_name: string }
}
...
function TabKasbonKenek({ showMessage }: { showMessage: (t: 'success' | 'error', msg: string) => void }) {
  ...
  async function fetchAll() {
    ...
    const [{ data: kData }, { data: allPerm }, { data: dedData }] = await Promise.all([
      ...
      supabase.from('helper_kasbon_deductions')
        .select('*, helper_kasbon(notes, total_amount), employees(full_name)')
        .order('week_start_date', { ascending: false })
        .limit(100),
    ])
    setKasbons((kData || []) as HelperKasbonRecord[])
  ...
  async function handleTambahKasbon(e: React.FormEvent) {
    e.preventDefault()
    const amt = parseFloat(form.total_amount)
    if (!form.helper_id) { showMessage('error', 'Pilih kenek terlebih dahulu.'); return }
    if (isNaN(amt) || amt <= 0) { showMessage('error', 'Nominal kasbon tidak valid.'); return }
    setSubmitting(true)

    const { error } = await supabase.from('helper_kasbon').insert({
  ...
  async function handleLunas(id: string) {
    if (!confirm('Tandai kasbon ini sebagai LUNAS?')) return
    const { error } = await supabase.from('helper_kasbon')
      .update({ status: 'lunas', remaining_amount: 0, updated_at: new Date().toISOString() })
      .eq('id', id)
    if (error) showMessage('error', 'Gagal: ' + error.message)
    else { showMessage('success', 'Kasbon ditandai lunas.'); fetchAll() }
  ...
  async function handleHapus(id: string) {
    if (!confirm('Hapus kasbon ini? Riwayat potongan terkait akan ikut terhapus.')) return
    await supabase.from('helper_kasbon').delete().eq('id', id)
    showMessage('success', 'Kasbon dihapus.')
    fetchAll()
  }
  ...
  return (
    <div className="space-y-4">
            ...
            {loading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      {['Kenek','Kode','Kasbon Awal','Sisa Kasbon','Keterangan','Status','Aksi'].map(h => (
                        <th key={h} className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase whitespace-nowrap">{h}</th>
                      ))}
                    </tr>
                  </thead>
      ...
      {activeSubTab === 'tambah' && (
        <div className="max-w-md">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <h3 className="font-semibold text-slate-800 mb-4">Tambah Kasbon Kenek</h3>
            <form onSubmit={handleTambahKasbon} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">Kenek <span className="text-red-500">*</span></label>
                <select required value={form.helper_id} onChange={e => setForm({...form, helper_id: e.target.value})}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white focus:ring-2 focus:ring-blue-500 outline-none">
                  <option value="">-- Pilih Kenek --</option>
                  {keneks.map(k => (
                    <option key={k.id} value={k.id}>{k.full_name} ({k.employee_code})</option>
                  ))}
                </select>
              </div>
              <div>
              ...
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 text-xs text-orange-700">
                Kasbon akan langsung aktif. Potongan mingguan dapat dilakukan melalui halaman <strong>Penggajian Driver → Overview Kenek → Detail & Cetak Slip</strong>.
              </div>
              <button type="submit" disabled={submitting}
                className="w-full py-2 bg-orange-600 hover:bg-orange-700 text-white text-sm font-semibold rounded-lg transition disabled:opacity-50">
                {submitting ? 'Menyimpan...' : '💾 Simpan Kasbon'}
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    {['Kenek','Keterangan Kasbon','Periode Minggu','Dipotong','Sisa Setelah'].map(h => (
                      <th key={h} className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {deductions.map(d => {
                    ...
                    return (
                      <tr key={d.id} className="hover:bg-slate-50/70 transition-colors">
                        ...
                        <td className="px-4 py-3 text-slate-500 text-xs">
                          {dd.helper_kasbon?.notes || '(tanpa keterangan)'} · {fmtRp(dd.helper_kasbon?.total_amount || 0)}
                        </td>
                        <td className="px-4 py-3 text-slate-600 text-xs whitespace-nowrap">{periodeLabel}</td>
                        <td className="px-4 py-3 font-bold text-red-600">-{fmtRp(d.deduction_amount)}</td>
                        <td className="px-4 py-3 text-orange-600 font-medium">{fmtRp(d.remaining_after)}</td>
